...
function getBestItemComboPossible(@tp, @inventory , @target) {
		...
		var path = getPathLength(_CELL[_MY_SELF], getCellToUseItemBasic(item , _CELL[target]));
			if (item!=WEAPON_B_LASER && path!=null && path <= _MP[_MY_SELF] && _ITEM_COOLDOWN[item]==0 && cost<=tp && dmg/cost>BestDmg/BestCost) {
				BestCost = cost;
				BestDmg = dmg;
				BestItem = item;
			}
...
function TryOneShoot(@entity) {
	...
	if (inArray(getChips(), CHIP_TELEPORTATION) && BestItemTp!=null && getPathLength(_CELL[_MY_SELF], CellToTele)<=_MP[_MY_SELF]) {
